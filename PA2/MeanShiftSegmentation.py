...
iter = 20 #change as required
radius = 60 #choose 60 or 90
...
def is_prev_centre(prev_centre, current_centre):
    ...
    if (center_diff<=iter):
        ...
        return 0
    #using the reduced image 1/10th the original Butterfly.jpg
image = cv2.imread('Butterfly_reduced.jpg', 1)
...
while(len(f_vector_list)>0):#temp_vector)>0
    ...
    while(is_prev_centre(prev_centre,current_centre)==0):
        ...
        lst = []
        index_list=[]
        #populate the lst and index_list with pixels that fall within the radius - 30,60,90
        print("-------------------------len(f_vector - )-------------------------------"+str(len(f_vector)))
        for x in range(0, len(init_f_vector)):
            diff = abs(current_centre - init_f_vector[x])
...
import cv2
cv2.imshow('clustered_image_60.jpg', cluster_image.astype('uint8'))
#cv2.imwrite('clustered_image_90_smaller.jpg', cluster_image.astype('uint8'))
cv2.waitKey(0)
